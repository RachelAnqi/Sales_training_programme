server = function(input, output) {
  ...
  data1_phase1 <- eventReactive(input$decision1_phase1_submit,{
    data_decision <- data.frame(
      ...
      pp_promotional_support = NULL,
      pp_sales_performance = NULL,
      pp_offer_attractiveness = NULL,
      real_sales = NULL,
      discount = NULL,
    ...
    for (j in 1:10) {
      for (q in 1:4){
        ...
        data_decision <- plyr::rbind.fill(data_decision,data.frame(
          ...
          pp_promotional_support = as.numeric((pp_info_by_hosp_product[[j]][[q]])$promotional_support),
          pp_sales_performance = as.numeric((pp_info_by_hosp_product[[j]][[q]])$sales_performance),
          pp_offer_attractiveness = as.numeric((pp_info_by_hosp_product[[j]][[q]])$offer_attractiveness),
