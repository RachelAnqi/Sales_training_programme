...
calculator1 <- function(input,phase){
...
calculator2 <- function(input,phase) {
  phase_product1 <- 0 
  phase_product2 <- 0 
  phase_product3 <- 0 
  phase_product4 <- 0
  ...
  for(i in 1:10){
    ...
    tmp <- c(as.numeric(input[[paste("p",phase,"_hosp",i,"_target_1",sep="")]]),
                 as.numeric(input[[paste("p",phase,"_hosp",i,"_target_2",sep="")]]),
                 as.numeric(input[[paste("p",phase,"_hosp",i,"_target_3",sep="")]]),
                 as.numeric(input[[paste("p",phase,"_hosp",i,"_target_4",sep="")]]))
    
    phase_product1 <- phase_product1 + tmp[1]
    phase_product2 <- phase_product1 + tmp[2]
    phase_product3 <- phase_product1 + tmp[3] 
    phase_product4 <- phase_product1 + tmp[4]
    
  }
  
  data <- c(ifelse(is.na(phase_product1),0,phase_product1),
            ifelse(is.na(phase_product2),0,phase_product2),
            ifelse(is.na(phase_product3),0,phase_product3),
            ifelse(is.na(phase_product4),0,phase_product4))
  data
}
                        ...
                        curve(curve36,y)} else if (
                          ...
                          curve(curve37,y)} else if (x==as.character(product_info_list$product[4])) {
                            curve(curve38,y)} else if (x==as.character(product_info_list$product[1])|
                              ...
                              curve(curve39,y)} else{curve(curve35,y)}},
...
report_data <- function(tmp,flm_data,null_report) {
  ...
  report3_mod1 <- report3_mod1 %>%
    gather(`因素`,value,`销售代表`,`计划时间分配(天)`,`实际时间分配(天)`,`推广费用(元)`) %>%
...
writeDown <- function(phase,report,report8){
  ...
  writeDataTable(wb, sheet = rsd_sheet_names[1],withFilter = F, tibble(销售业绩 = " "),
                 startCol = 1,startRow = 7,rowNames = F,colNames = T)
  ...
  writeDataTable(wb, sheet = rsd_sheet_names[1],withFilter = F, report7_2,
                 startCol = 2,startRow = 7,rowNames = F,colNames = T)
  ...
  writeDataTable(wb, sheet = rsd_sheet_names[7],withFilter = F, tibble("销售额/客户" = " "),
                 startRow = 1,rowNames = F,colNames = T)
  report6_1 <- report$report6_mod1
  writeDataTable(wb, sheet = rsd_sheet_names[7],withFilter = F, report6_1,
                 startCol = 2,rowNames = F,colNames = T)
  writeDataTable(wb, sheet = rsd_sheet_names[7],withFilter = F, tibble("销售额/代表" = " "),
                 startCol = 1,startRow = 63,rowNames = F,colNames = T)
  report6_2 <-report$report6_mod2
  writeDataTable(wb, sheet = rsd_sheet_names[7],withFilter = F, report6_2,
                 startCol = 2,startRow = 63,rowNames = F,colNames = T)
  writeDataTable(wb, sheet = rsd_sheet_names[7],withFilter = F, tibble("销售额/产品" = " "),
                 startCol = 1,startRow = 96,rowNames = F,colNames = T)
  ...
  writeDataTable(wb, sheet = rsd_sheet_names[7],withFilter = F, report6_3,
                 startCol = 2,startRow = 96,rowNames = F,colNames = T)
